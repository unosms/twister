...
<div class="flex h-screen overflow-hidden">
    ...
    <form class="mt-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm" method="get" action="{{ route('devices.graphs') }}" data-graph-filter-form>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-2">
            ...
            @endforeach
            @if($isAdminRole)
                <a class="rounded-full bg-slate-100 px-4 py-2 text-xs font-black text-slate-600 hover:bg-slate-200" href="{{ route('devices.events.index', ['device_id' => $device->id]) }}">Back to Events</a>
            @endif
        </div>
    </form>
