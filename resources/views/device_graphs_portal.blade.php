...
@php
    ...
    $backRouteLabel = 'Back to Portal';
    $backRouteUrl = route('portal.index');
...
<main class="min-h-screen overflow-y-auto">
        <header class="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur sm:px-6 lg:px-8">
            <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Portal</p>
    ...
    <form class="mt-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm" method="get" action="{{ route('portal.devices.graphs') }}" data-graph-filter-form>
        <div class="grid gap-4 lg:grid-cols-12">
            ...
            @foreach($unitOptions as $unitKey => $unitOption)
                <a class="rounded-full px-4 py-2 text-xs font-black {{ $unit === $unitKey ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}" href="{{ route('portal.devices.graphs') . '?' . http_build_query(array_merge($queryBase, ['unit' => $unitKey])) }}">{{ $unitOption['label'] }}</a>
            @endforeach
            <a class="rounded-full bg-slate-100 px-4 py-2 text-xs font-black text-slate-600 hover:bg-slate-200" href="{{ route('portal.index') }}">Back to Portal</a>
        </div>
    </form>
