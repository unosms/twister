#!/usr/bin/env bash
# /etc/scripts/showmacnexus.sh
# Env: SWITCH_IP, SWITCH_USERNAME, SWITCH_PASS, INTERFACE
# Example:
#   SWITCH_IP=192.168.200.244 SWITCH_USERNAME=admin SWITCH_PASS='pass' INTERFACE='Eth1/1' /etc/scripts/showmacnexus.sh
...
expect <<'EXP'
    # Enable internal debug with: DEBUG=1 /etc/scripts/showmacnexus.sh
    ...
    # Connect via telnet (say if you want SSH instead)
    spawn telnet $ip
    ...
    # Show MACs on the given interface (NX-OS)
